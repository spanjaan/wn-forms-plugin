...
namespace SpAnjaan\Forms\Controllers;
...
class Records extends Controller {
    ...
    public $requiredPermissions = ['spanjaan.forms.access_records'];
    ...
    public function __construct() {
        ...
        $group = get('group');
        if(!empty($group)) {
            BackendMenu::setContext('SpAnjaan.Forms', 'forms', str_slug($group));
        } else {
            BackendMenu::setContext('SpAnjaan.Forms', 'forms', 'records');
        }
    }
    ...
    public function onDeleteSingle() {
        ...
        $record = Record::findOrFail($id);
        $record->delete();
        return Redirect::to(Backend::url('spanjaan/forms/records'));
    }
    ...
    public function onGDPRClean() {
        if ($this->user->hasPermission(['spanjaan.forms.gdpr_cleanup'])) {
            MagicForm::gdprClean();
